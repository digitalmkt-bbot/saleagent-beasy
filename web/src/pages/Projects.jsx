...
export default function Projects() {
  ...
  return (
    <div>
      <h1 className="page">{t('โครงการ')}</h1>
      <div className="cards">
        ...
        <div className="card"><div className="label">{t('จำนวนโครงการ')}</div><div className="value">{data.rows.length}</div></div>
      ...
      <div className="panel">
        <table><thead><tr><th>{t('รหัส')}</th><th>{t('โครงการ')}</th><th>{t('ลูกค้า')}</th><th>{t('สำคัญ')}</th><th>{t('กิจกรรม')}</th><th>{t('วันเริ่ม-สิ้นสุด')}</th><th>{t('สถานะ (ไปป์ไลน์)')}</th><th>{t('มูลค่า')}</th><th>{t('ผู้รับผิดชอบ')}</th><th>{t('เลื่อน')}</th></tr></thead>
          <tbody>{data.rows.map(p => (
            ...
            {!data.rows.length && <tr><td colSpan="10" className="muted">{t('ไม่มีโครงการ')}</td></tr>}
          </tbody></table>
      </div>
...
function ProjectModal({ meta, t, onClose, onSaved }) {
  ...
  const PR = ['ต่ำ', 'ปานกลาง', 'สูง', 'สูงมาก', 'ด่วนที่สุด'];
  async function save() {
    if (!f.name) return setErr(t('กรอกชื่อโครงการ'));
