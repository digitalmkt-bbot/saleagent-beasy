...
function DotLine() { const v=[30,26,33,30,42,40,54,66],W=220,H=58,mx=66,mn=26; const xs=i=>4+i/(v.length-1)*(W-8), ys=x=>H-8-((x-mn)/((mx-mn)||1))*(H-16); const pts=v.map((x,i)=>[xs(i),ys(x)]); const ln=smoothPath(pts); return <svg viewBox="0 0 220 58" style={{ width: '100%', height: 58 }}><path d={ln} fill="none" stroke={VIO} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />{pts.map((p,i)=><circle key={i} cx={p[0].toFixed(1)} cy={p[1].toFixed(1)} r="2.3" fill={VIO} />)}</svg>; }
