...
const ATYPES = ['โทรติดตาม', 'นัดหมาย', 'เข้าพบ/นำเสนอ', 'ส่งใบเสนอราคา', 'ติดตามชำระเงิน', 'อื่นๆ'];

function DotWave() { const W = 220, H = 56, d = []; for (let i = 0; i < 64; i++) { const x = 2 + i / 63 * (W - 4), y = H / 2 + Math.sin(i / 5) * 14 + Math.sin(i * 1.7) * 4, t = i / 63; d.push(<circle key={i} cx={x.toFixed(1)} cy={y.toFixed(1)} r="1.6" fill={`rgb(${Math.round(99 + t * 20)},${Math.round(102 + t * 100)},${Math.round(241 - t * 30)})`} opacity="0.85" />); } return <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 56 }}>{d}</svg>; }
function DotGrid() { const c = []; for (let r = 0; r < 5; r++) for (let k = 0; k < 18; k++) { const op = Math.max(0.12, Math.min(1, 0.18 + (r / 4) * 0.55 + 0.22 * Math.sin(k / 2 + r))); c.push(<circle key={r + '-' + k} cx={6 + k * 12} cy={8 + r * 11} r="2.3" fill={IND} opacity={op.toFixed(2)} />); } return <svg viewBox="0 0 220 56" style={{ width: '100%', height: 56 }}>{c}</svg>; }
function AreaMini({ data }) { const v = (data && data.length ? data.map(d => d.value) : [20, 30, 26, 40, 52, 44, 58, 50, 64]); const W = 220, H = 56, mx = Math.max(1, ...v); const xs = i => (v.length === 1 ? W / 2 : i / (v.length - 1) * W), ys = x => H - 4 - (x / mx) * (H - 10); const ln = v.map((x, i) => `${i ? 'L' : 'M'} ${xs(i).toFixed(1)} ${ys(x).toFixed(1)}`).join(' '); const ar = `M 0 ${H} ` + v.map((x, i) => `L ${xs(i).toFixed(1)} ${ys(x).toFixed(1)}`).join(' ') + ` L ${W} ${H} Z`; return <svg viewBox="0 0 220 56" style={{ width: '100%', height: 56 }}><defs><linearGradient id="amg" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor={TEAL} stopOpacity="0.5" /><stop offset="1" stopColor={TEAL} stopOpacity="0.05" /></linearGradient></defs><path d={ar} fill="url(#amg)" /><path d={ln} fill="none" stroke={TEAL} strokeWidth="2" /></svg>; }
function DotLine() { const v = [42, 32, 26, 24, 27, 30, 38, 50], W = 220, H = 56, mx = 50; const xs = i => 4 + i / (v.length - 1) * (W - 8), ys = x => H - 6 - (x / mx) * (H - 14); const ln = v.map((x, i) => `${i ? 'L' : 'M'} ${xs(i).toFixed(1)} ${ys(x).toFixed(1)}`).join(' '); return <svg viewBox="0 0 220 56" style={{ width: '100%', height: 56 }}><path d={ln} fill="none" stroke={IND} strokeWidth="2" strokeDasharray="1 4" strokeLinecap="round" />{v.map((x, i) => <circle key={i} cx={xs(i).toFixed(1)} cy={ys(x).toFixed(1)} r="2.4" fill={IND} />)}</svg>; }
function BarsMini() { const v = [30, 44, 38, 56, 48, 62, 54, 70, 60, 78], H = 56, mx = 78, bw = 14; return <svg viewBox="0 0 220 56" style={{ width: '100%', height: 56 }}><defs><linearGradient id="bmg" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor={TEAL} /><stop offset="1" stopColor={GRN} stopOpacity="0.6" /></linearGradient></defs>{v.map((x, i) => { const h = (x / mx) * (H - 8); return <rect key={i} x={4 + i * (bw + 6)} y={H - h} width={bw} height={h} rx="4" fill="url(#bmg)" />; })}</svg>; }
...
export default function Dashboard() {
  ...
  const onTime = doneSum + overSum ? Math.round(doneSum / (doneSum + overSum) * 100) : 80;
