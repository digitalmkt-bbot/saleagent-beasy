...
router.get('/notifications', wrap(async (req, res) => {
  const rows = await q(
    `SELECT a.id, a.detail, a.due_at, a.status, c.name AS customer_name, u.display_name AS assignee_name,
     ...
     ORDER BY a.due_at ASC LIMIT 50`, [cid(req)]);
  res.json({ rows: rows.rows, count: rows.rows.length });
}));

router.get('/dashboard', wrap(async (req, res) => {
  const c = cid(req);
  const cust = await q(`SELECT count(*)::int total, count(*) FILTER (WHERE lifecycle_stage='new')::int new FROM customer WHERE company_id=$1`, [c]);
  const proj = await q(`SELECT count(*)::int total, count(*) FILTER (WHERE is_open)::int open,
     ...
     COALESCE(sum(estimated_value) FILTER (WHERE NOT is_open),0) won_value FROM project WHERE company_id=$1`, [c]);
  const act = await q(`SELECT count(*) FILTER (WHERE status='pending')::int pending,
     count(*) FILTER (WHERE due_at::date<CURRENT_DATE AND status='pending')::int overdue FROM activity WHERE company_id=$1`, [c]);
  const byStage = await q(`SELECT s.seq, s.name, count(p.id)::int cnt, COALESCE(sum(p.estimated_value),0) value
     FROM pipeline_stage s LEFT JOIN project p ON p.stage_id=s.id AND p.is_open
