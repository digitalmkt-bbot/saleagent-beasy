...
router.get('/', wrap(async (req, res) => {
  const rows = (await q(`SELECT u.id, u.user_code, u.display_name, u.email, u.phone, u.role, u.is_active, u.team_id, t.name AS team_name, u.created_at
...
router.post('/', wrap(async (req, res) => {
  ...
  try {
    const r = await q(`INSERT INTO app_user (company_id,user_code,display_name,email,phone,password_hash,role,team_id,is_active)
      VALUES ($1,$2,$3,$4,$5,$6,COALESCE($7,'sales'),$8,true) RETURNING id,user_code,display_name,email,phone,role,is_active,team_id`,
      [req.user.company_id, (b.user_code||'').trim()||null, b.display_name, String(b.email).toLowerCase().trim(), b.phone || null, hash, b.role || null, b.team_id || null]);
    res.status(201).json(r.rows[0]);
  } catch (e) {
    if (e.code === '23505') return res.status(409).json({ error: String(e.message).includes('user_code') ? 'รหัสผู้ใช้ (ID) นี้ถูกใช้แล้ว' : 'อีเมลนี้ถูกใช้แล้ว' });
    throw e;
  }
}));
...
router.put('/:id', wrap(async (req, res) => {
  ...
  try {
    const r = await q(`UPDATE app_user SET user_code=$9, display_name=$3, email=$4, phone=$5, role=$6, team_id=$7, is_active=$8
      ...
      [id, req.user.company_id, b.display_name, String(b.email).toLowerCase().trim(), b.phone || null, b.role || 'sales', b.team_id || null, b.is_active !== false, (b.user_code||'').trim()||null]);
